    ...
    <main className="min-h-screen bg-white text-[#0E0E2C]">
      ...
      <section className="pt-36 pb-16 px-8">
        <div className="mx-auto max-w-7xl">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#0E0E2C]/40 mb-3">Use Cases</p>
      ...
      <section className="pb-24 px-8">
        <div className="mx-auto max-w-7xl grid grid-cols-1 md:grid-cols-3 gap-6">
          {useCases.map((uc) => (
            <div key={uc.tag} className="rounded-2xl border border-[#0E0E2C]/8 bg-[#FCF9EA] p-8 hover:border-[#0E0E2C]/20 transition-colors flex flex-col gap-4">
              <span className="inline-block text-xs font-semibold uppercase tracking-widest bg-[#0E0E2C]/6 text-[#0E0E2C]/50 px-2.5 py-1 rounded-full w-fit">{uc.tag}</span>
              <h2 className="text-lg font-bold">{uc.title}</h2>
              <p className="text-sm text-[#0E0E2C]/60 leading-relaxed">{uc.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 px-8 bg-[#FCF9EA] text-center">
        ...
        <p className="text-[#0E0E2C]/60 mb-6 text-sm">Talk to us. Lawrora is flexible enough to fit almost any legal workflow.</p>
